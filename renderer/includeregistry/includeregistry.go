...
// Register adds a new FileSystem to the include registry.  Duplicates (seq, name or value) are not detected or prevented.
func Register(seq float64, name string, includeFS http.FileSystem) {
...
// not an http.FileSystem.  If overrideFS is not nil, it will be included as the
...
func MakeFS(overrideFS http.FileSystem, contents webutil.NamedSequence, debug bool) http.FileSystem {
	...
	fslist := make([]http.FileSystem, 0, len(l)+1)
	if overrideFS != nil {
		...
		if debug {
			fs = fsutil.NewHTTPFuncFS(func(name string) (http.File, error) {
				...
				if f != nil {
					log.Printf("Include file opened from override: %s", name)
				}
				return f, err
			})
		}
